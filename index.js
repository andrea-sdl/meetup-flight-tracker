...
async function getAirfarePrice(origin) {
  try {
    ...
    if (response?.data && response.data.length > 0 ){
      ...
      console.log(`Airfare price from ${origin} to ${DESTINATION} on ${DEPARTURE_DATE} - ${RETURN_DATE}: $${price}`);
      return price-0;
    }
    console.error(`[NOT-FOUND] Airfare price from ${origin} to ${DESTINATION} on ${DEPARTURE_DATE} - ${RETURN_DATE} could not be found`);
    return 0;
  } catch (error) {
    console.error(error);
    return 0;
...
async function main() {
  ...
  for await (const origin of rl) {
    const price = await getAirfarePrice(origin.trim());
    totalPrice += price;
  }

  console.log(`Total airfare price from all origins in ${ORIGINS_FILE} to ${DESTINATION} on ${DEPARTURE_DATE} - ${RETURN_DATE}: $${totalPrice}`);
}
